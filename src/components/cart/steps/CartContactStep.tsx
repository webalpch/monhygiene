...
export const CartContactStep: React.FC<CartContactStepProps> = ({
  ...
  const validateForm = () => {
    ...
    if (!formData.phone.trim()) {
      ...
      newErrors.phone = 'Le numéro de téléphone doit contenir au moins 10 caractères';
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Finalisez votre commande</h2>
        <p className="text-gray-600">Vos informations de contact pour confirmer la réservation</p>
      </div>
        ...
        <div className="lg:col-span-1">
          <Card className="sticky top-4">
            ...
            <CardContent className="space-y-4">
              {/* Address */}
              <div>
                <h4 className="font-medium text-sm mb-1">Adresse</h4>
              ...
              {selectedSlot && (
                <div>
                  <h4 className="font-medium text-sm mb-1">Date & horaire</h4>
              ...
              {/* Services */}
              <div>
                <h4 className="font-medium text-sm mb-2">Services ({cartItems.length})</h4>
                <div className="space-y-2">
                  {cartItems.map((item) => {
                    ...
                    if (item.service.id === 'nettoyage-canape' && item.formData?.numberOfSeats) {
                      serviceName += ` (${item.formData.numberOfSeats} place${item.formData.numberOfSeats > 1 ? 's' : ''})`;
                    }
                    ...
                    return (
                      <div key={item.id} className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">
                          {servicesOnQuote.includes(item.service.id) ? 'Sur devis' : `${item.estimatedPrice} CHF`}
                        </span>
                      </div>
                    );
                  })}
              ...
              <div className="border-t pt-3">
                ...
                {cartItems.some(item => ['nettoyage-terrasse', 'nettoyage-toiture', 'autres-services', 'nettoyage-vitres', 'nettoyage-moquette-tapis'].includes(item.service.id)) && (
                  <p className="text-xs text-gray-500 mt-1">
                    * Services "sur devis" non inclus dans le total
                  </p>
                )}
              </div>
